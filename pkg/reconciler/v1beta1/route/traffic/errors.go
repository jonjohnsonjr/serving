...
type TargetError interface {
	...
	MarkBadTrafficTarget(rs *v1beta1.RouteStatus)
...
func (e *missingTargetError) MarkBadTrafficTarget(rs *v1beta1.RouteStatus) {
...
// MarkBadTrafficTarget implements TargetError.
func (e *unreadyConfigError) MarkBadTrafficTarget(rs *v1beta1.RouteStatus) {
	if e.IsFailure() {
...
// MarkBadTrafficTarget implements TargetError.
func (e *unreadyRevisionError) MarkBadTrafficTarget(rs *v1beta1.RouteStatus) {
	if e.IsFailure() {
...
func errUnreadyConfiguration(config *v1beta1.Configuration) TargetError {
	status := corev1.ConditionUnknown
	if c := config.Status.GetCondition(v1beta1.ConfigurationConditionReady); c != nil {
		status = c.Status
	}
...
func errUnreadyRevision(rev *v1beta1.Revision) TargetError {
	status := corev1.ConditionUnknown
	if c := rev.Status.GetCondition(v1beta1.RevisionConditionReady); c != nil {
		status = c.Status
	}
