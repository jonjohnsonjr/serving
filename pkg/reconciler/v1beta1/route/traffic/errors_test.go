...
func TestMarkBadTrafficTarget_Missing(t *testing.T) {
	err := errMissingRevision("missing-rev")
	r := getTestRouteWithTrafficTargets([]v1beta1.TrafficTarget{})

	err.MarkBadTrafficTarget(&r.Status)
	for _, condType := range []v1beta1.RouteConditionType{
		v1beta1.RouteConditionAllTrafficAssigned,
		v1beta1.RouteConditionReady,
	} {
		got := r.Status.GetCondition(condType)
		want := &v1beta1.RouteCondition{
			Type:               condType,
