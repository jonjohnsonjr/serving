...
import (
	...
	listers "github.com/knative/serving/pkg/client/listers/serving/v1beta1"
...
var (
	...
	missingConfig *v1beta1.Configuration
	missingRev    *v1beta1.Revision

	// emptyConfig never has any revision.
	emptyConfig *v1beta1.Configuration

	// revDeletedConfig has a Ready revision but was deleted.
	revDeletedConfig *v1beta1.Configuration

	// unreadyConfig only has unreadyRev, and it's not ready.
	unreadyConfig *v1beta1.Configuration
	unreadyRev    *v1beta1.Revision

	// failedConfig only has failedRev, and it fails to be ready.
	failedConfig *v1beta1.Configuration
	failedRev    *v1beta1.Revision

	// inactiveConfig only has inactiveRevision, and it's not active.
	inactiveConfig *v1beta1.Configuration
	inactiveRev    *v1beta1.Revision

	// goodConfig has two good revisions: goodOldRev and goodNewRev
	goodConfig *v1beta1.Configuration
	goodOldRev *v1beta1.Revision
	goodNewRev *v1beta1.Revision

	// niceConfig has two good revisions: niceOldRev and niceNewRev
	niceConfig *v1beta1.Configuration
	niceOldRev *v1beta1.Revision
	niceNewRev *v1beta1.Revision
...
func TestBuildTrafficConfiguration_Vanilla(t *testing.T) {
	...
	expected := &TrafficConfig{
		...
		Configurations: map[string]*v1beta1.Configuration{goodConfig.Name: goodConfig},
		Revisions:      map[string]*v1beta1.Revision{goodNewRev.Name: goodNewRev},
	}
...
func TestBuildTrafficConfiguration_VanillaScaledToZero(t *testing.T) {
	...
	expected := &TrafficConfig{
		...
		Configurations: map[string]*v1beta1.Configuration{inactiveConfig.Name: inactiveConfig},
		Revisions:      map[string]*v1beta1.Revision{inactiveRev.Name: inactiveRev},
	}
	if tc, err := BuildTrafficConfiguration(configLister, revLister, getTestRouteWithTrafficTargets(tts)); err != nil {
		t.Errorf("Unexpected error: %v", err)
	} else if diff := cmp.Diff(expected, tc); diff != "" {
...
func TestBuildTrafficConfiguration_TwoConfigs(t *testing.T) {
	...
	expected := &TrafficConfig{
		...
		Configurations: map[string]*v1beta1.Configuration{goodConfig.Name: goodConfig, niceConfig.Name: niceConfig},
		Revisions:      map[string]*v1beta1.Revision{goodNewRev.Name: goodNewRev, niceNewRev.Name: niceNewRev},
	}
	if tc, err := BuildTrafficConfiguration(configLister, revLister, getTestRouteWithTrafficTargets(tts)); err != nil {
		t.Errorf("Unexpected error %v", err)
	} else if diff := cmp.Diff(expected, tc); diff != "" {
...
func TestBuildTrafficConfiguration_Canary(t *testing.T) {
	...
	expected := &TrafficConfig{
		...
		Configurations: map[string]*v1beta1.Configuration{goodConfig.Name: goodConfig},
		Revisions:      map[string]*v1beta1.Revision{goodOldRev.Name: goodOldRev, goodNewRev.Name: goodNewRev},
	}
	if tc, err := BuildTrafficConfiguration(configLister, revLister, getTestRouteWithTrafficTargets(tts)); err != nil {
		t.Errorf("Unexpected error %v", err)
	} else if diff := cmp.Diff(expected, tc); diff != "" {
...
func TestBuildTrafficConfiguration_TwoFixedRevisions(t *testing.T) {
	...
	expected := &TrafficConfig{
		...
		Configurations: map[string]*v1beta1.Configuration{goodConfig.Name: goodConfig},
		Revisions:      map[string]*v1beta1.Revision{goodNewRev.Name: goodNewRev, goodOldRev.Name: goodOldRev},
	}
	if tc, err := BuildTrafficConfiguration(configLister, revLister, getTestRouteWithTrafficTargets(tts)); err != nil {
		t.Errorf("Unexpected error %v", err)
	} else if diff := cmp.Diff(expected, tc); diff != "" {
...
func TestBuildTrafficConfiguration_Preliminary(t *testing.T) {
	...
	expected := &TrafficConfig{
		...
		Configurations: map[string]*v1beta1.Configuration{goodConfig.Name: goodConfig, niceConfig.Name: niceConfig},
		Revisions:      map[string]*v1beta1.Revision{goodOldRev.Name: goodOldRev, goodNewRev.Name: goodNewRev, niceNewRev.Name: niceNewRev},
	}
	if tc, err := BuildTrafficConfiguration(configLister, revLister, getTestRouteWithTrafficTargets(tts)); err != nil {
		t.Errorf("Unexpected error %v", err)
	} else if diff := cmp.Diff(expected, tc); diff != "" {
...
func TestBuildTrafficConfiguration_NotRoutableRevision(t *testing.T) {
	...
	expected := &TrafficConfig{
		Targets:        map[string][]RevisionTarget{},
		Configurations: map[string]*v1beta1.Configuration{},
		Revisions:      map[string]*v1beta1.Revision{unreadyRev.Name: unreadyRev},
	}
...
func TestBuildTrafficConfiguration_NotRoutableConfiguration(t *testing.T) {
	...
	expected := &TrafficConfig{
		Targets:        map[string][]RevisionTarget{},
		Configurations: map[string]*v1beta1.Configuration{unreadyConfig.Name: unreadyConfig},
		Revisions:      map[string]*v1beta1.Revision{},
	}
...
func TestBuildTrafficConfiguration_EmptyConfiguration(t *testing.T) {
	...
	expected := &TrafficConfig{
		Targets:        map[string][]RevisionTarget{},
		Configurations: map[string]*v1beta1.Configuration{emptyConfig.Name: emptyConfig},
		Revisions:      map[string]*v1beta1.Revision{},
	}
...
func TestRoundTripping(t *testing.T) {
	...
	expected := []v1beta1.TrafficTarget{{
		ConfigurationName: goodConfig.Name,
		RevisionName:      goodOldRev.Name,
		Percent:           100,
	}, {
...
func getTestConfig(name string) *v1beta1.Configuration {
...
func getTestRevForConfig(config *v1beta1.Configuration, name string) *v1beta1.Revision {
...
func getTestEmptyConfig(name string) *v1beta1.Configuration {
	config := getTestConfig(name + "-config")
	config.Status.InitializeConditions()
	return config
}

func getTestConfigWithDeletedRevision(name string) *v1beta1.Configuration {
	config := getTestConfig(name + "-config")
...
func getTestUnreadyConfig(name string) (*v1beta1.Configuration, *v1beta1.Revision) {
	config := getTestConfig(name + "-config")
...
func getTestFailedConfig(name string) (*v1beta1.Configuration, *v1beta1.Revision) {
	config := getTestConfig(name + "-config")
	rev := getTestRevForConfig(config, name+"-revision")
	config.Status.SetLatestCreatedRevisionName(rev.Name)
...
func getTestInactiveConfig(name string) (*v1beta1.Configuration, *v1beta1.Revision) {
	config := getTestConfig(name + "-config")
	rev := getTestRevForConfig(config, name+"-revision")
...
func getTestReadyConfig(name string) (*v1beta1.Configuration, *v1beta1.Revision, *v1beta1.Revision) {
	config := getTestConfig(name + "-config")
