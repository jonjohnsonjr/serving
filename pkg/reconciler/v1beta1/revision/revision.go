...
import (
	...
	kpainformers "github.com/knative/serving/pkg/client/informers/externalversions/autoscaling/v1beta1"
	servinginformers "github.com/knative/serving/pkg/client/informers/externalversions/serving/v1beta1"
	...
	kpalisters "github.com/knative/serving/pkg/client/listers/autoscaling/v1beta1"
...
func NewController(
	...
	deploymentInformer.Informer().AddEventHandler(cache.FilteringResourceEventHandler{
		FilterFunc: controller.Filter(v1beta1.SchemeGroupVersion.WithKind("Revision")),
...
func (c *Reconciler) reconcile(ctx context.Context, rev *v1beta1.Revision) error {
	logger := logging.FromContext(ctx)
	...
	if rev.Spec.BuildName != "" {
		...
		before := rev.Status.GetCondition(v1beta1.RevisionConditionBuildSucceeded)
		rev.Status.PropagateBuildStatus(build.Status)
		after := rev.Status.GetCondition(v1beta1.RevisionConditionBuildSucceeded)
	...
	bc := rev.Status.GetCondition(v1beta1.RevisionConditionBuildSucceeded)
	if bc == nil || bc.Status == corev1.ConditionTrue {
		// There is no build, or the build completed successfully.

		phases := []struct {
			name string
			f    func(context.Context, *v1beta1.Revision) error
		}{{
...
func (c *Reconciler) updateRevisionLoggingURL(rev *v1beta1.Revision) {
...
func (c *Reconciler) updateStatus(rev *v1beta1.Revision) (*v1beta1.Revision, error) {
