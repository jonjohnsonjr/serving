...
func getTestReadyKPA(rev *v1beta1.Revision) *kpa.PodAutoscaler {
...
func TestResolutionFailed(t *testing.T) {
	...
	for _, ct := range []v1beta1.RevisionConditionType{"ContainerHealthy", "Ready"} {
		got := rev.Status.GetCondition(ct)
		want := &v1beta1.RevisionCondition{
			Type:               ct,
...
func TestCreateRevWithCompletedBuildNameCompletes(t *testing.T) {
	...
	for _, ct := range []v1beta1.RevisionConditionType{"BuildSucceeded"} {
...
func TestMarkRevReadyUponEndpointBecomesReady(t *testing.T) {
	...
	for _, ct := range []v1beta1.RevisionConditionType{"Ready"} {
